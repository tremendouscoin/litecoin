...
const char * const BITCOIN_CONF_FILENAME = "tremendouscoin.conf";
const char * const BITCOIN_PID_FILENAME = "tremendouscoin.pid";
...
#else
    const char* pszModule = "tremendouscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tremendouscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tremendouscoin";
#else
    // Unix
    return pathRet / ".tremendouscoin";
#endif
#endif
}
